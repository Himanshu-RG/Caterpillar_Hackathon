    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-cat-border/60">
        <div>
          ...
          <h1 className="text-2xl font-black text-white">Operational Incident Logs</h1>
      ...
      {successBanner && (
        <div className="rounded-lg bg-emerald-950/80 border border-emerald-600/70 p-3.5 flex items-center gap-2 text-xs font-bold text-emerald-300 animate-in fade-in">
      ...
      <Card
        ...
        badge={<span className="font-mono text-xs text-slate-400">{incidents.length} total</span>}
      >
        {loading ? (
          ...
          <div className="py-12 text-center text-slate-500">
            ...
            <p className="text-xs font-semibold text-slate-400">Zero logged incidents recorded</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-800">
                {incidents.map((inc) => (
                  <tr key={inc.incident_id} className="hover:bg-slate-900/60">
                    <td className="py-3 px-3 text-slate-400 font-bold">{inc.incident_id}</td>
                    <td className="py-3 px-3 text-slate-400">{inc.timestamp.slice(0, 19).replace('T', ' ')}</td>
                    <td className="py-3 px-3 text-white font-bold">{inc.machine_id}</td>
                    <td className="py-3 px-3 text-slate-300">{inc.operator_id}</td>
                    <td className="py-3 px-3 text-slate-200">{inc.incident_type.replace(/_/g, ' ')}</td>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white font-mono focus:border-cat-yellow focus:outline-none"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white focus:border-cat-yellow focus:outline-none"
                  >
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  className="w-full bg-slate-900 border border-slate-700 rounded p-2.5 text-white placeholder-slate-500 focus:border-cat-yellow focus:outline-none resize-none"
                />
              </div>
