...
export const MetricCard: React.FC<MetricCardProps> = ({
  ...
  const statusColors = {
    normal: 'text-emerald-400 border-emerald-900/40',
    warning: 'text-amber-400 border-amber-900/50 shadow-glow-amber',
    critical: 'text-rose-400 border-rose-900/60 shadow-glow-red animate-pulse-fast',
    telemetry: 'text-cyan-400 border-cat-border/80',
  };

  const statusBg = {
    normal: 'bg-emerald-950/20',
    warning: 'bg-amber-950/20',
    critical: 'bg-rose-950/30',
    telemetry: 'bg-cat-surface-card/90',
  };

  return (
    <div
      className={clsx(
        'industrial-card rounded-lg p-3.5 flex flex-col justify-between transition-all duration-200 border',
        statusColors[status],
        statusBg[status],
        className
      )}
    >
      ...
      <div className="flex items-baseline gap-1.5 my-1">
        <span className="font-mono-num text-3xl font-extrabold tracking-tight text-white">
          {formattedValue}
        </span>
        {unit && (
      ...
      <div className="flex items-center justify-between pt-1 border-t border-slate-800/60 text-[11px] text-slate-400">
        {trend && (
          <div className="flex items-center gap-1">
            {trend === 'up' && <ArrowUpRight className="w-3.5 h-3.5 text-amber-400" />}
            {trend === 'down' && <ArrowDownRight className="w-3.5 h-3.5 text-cyan-400" />}
