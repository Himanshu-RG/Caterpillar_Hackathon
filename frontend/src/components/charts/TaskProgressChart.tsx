...
import { Clock, TrendingUp, TrendingDown, Target, CheckCircle2 } from 'lucide-react';
...
export const TaskProgressChart: React.FC<TaskProgressChartProps> = ({
  ...
  if (!task) {
    return (
      <div className="industrial-card rounded-lg p-5 text-center text-slate-500">
        <Target className="w-8 h-8 mx-auto mb-2 text-slate-600" />
        <p className="text-xs font-semibold">No Active Earthmoving Task</p>
        <p className="text-[11px] text-slate-500 mt-0.5">Machine is in standby or transit</p>
      </div>
    );
  }
  ...
  const actualMin = task.actual_time_min || 32;
    ...
    <div className="industrial-card rounded-lg p-4">
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          ...
          <h3 className="text-sm font-bold text-white mt-0.5">{task.task_type}</h3>
          <span className="text-xs font-mono text-slate-400">ID: {task.task_id}</span>
        </div>
      ...
      <div className="space-y-1.5 mb-4">
        <div className="flex items-baseline justify-between text-xs">
          ...
          <span className="font-mono-num font-bold text-cat-yellow">{pct}%</span>
        </div>
      ...
      {/* Dynamic ETA Block */}
      <div className="rounded-lg bg-slate-900/90 border border-cat-border p-3 mb-3">
        ...
        <div className="flex items-baseline justify-between">
          <div>
            <span className="font-mono-num text-2xl font-black text-white">
              {remainingMin} min
            </span>
          ...
          <div className="text-right">
            ...
            <span className="font-mono text-sm font-bold text-cat-yellow">{etaString}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="rounded bg-slate-900/60 border border-slate-800 p-2">
          <span className="text-[10px] text-slate-400 uppercase font-semibold block">
            Cycle Time
          </span>
          <div className="flex items-center justify-between mt-1">
            <span className="font-mono text-sm font-bold text-white">{currentCycleTime} sec</span>
        ...
        <div className="rounded bg-slate-900/60 border border-slate-800 p-2">
          ...
          <div className="flex items-center justify-between mt-1">
            <span className="font-mono text-sm font-bold text-white">{currentPayload.toFixed(1)} t</span>
